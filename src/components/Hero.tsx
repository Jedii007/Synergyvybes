    ...
    <div className="flex flex-col md:flex-row gap-4 p-4 min-h-screen w-full">
      {heroData.cards.map((card, index) => (
        <Link
          key={index}
          href={card.href}
          className={`relative flex-1 h-[50vh] md:h-[90vh] rounded-3xl overflow-hidden transition-all duration-${CARD_ANIMATION_DURATION} hover:scale-[1.02] bg-gray-200 cursor-pointer
